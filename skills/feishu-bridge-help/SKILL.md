...
```
用户发消息 → 👀 处理中 → [AI 回复内容] → ✅ 完成  ↑总输入 ↓总输出 ⚡缓存/未命中 📊上下文 🔄次数 ¥本次/当日
```
...
- **提示词文件** `.pi/feishu-prompt.md`：定义机器人行为，每次对话自动注入
- **记忆文件** `.pi/feishu-memory.md`：长期记忆，机器人可读写
...
| `/feishu-prompt` | 查看/编辑提示词文件 |
| `/feishu-memory` | 查看/编辑记忆文件 |
...
rm -rf .pi/ sessions/ workspaces/  # 清理数据
```
...
```
飞书 App ──WebSocket──→ pi-feishu-bridge ──SDK──→ pi Agent (LLM)
    ↑                        │
    │               ┌────────┴────────┐
    └── 消息回复 ───┤  sessions/      │
                   │  workspaces/     │
